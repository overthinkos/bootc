...
The **bootc bootable-container image family** for [OpenCharly](https://github.com/overthinkos/overthink),
...
| Kind | Entries |
|---|---|
| `image:` | `bazzite` (ublue Bazzite NVIDIA AI/dev workstation — the canonical worked example), `aurora` (ublue Aurora DX + charly toolchain) |
...
- the main repo is mounted under the `charly` namespace via `import:`, bringing
  `charly.fedora-builder` into scope — bootc images root on external bases so they
  inherit no builder map and fall through to `defaults.builder → charly.fedora-builder`.

### Tag pins, each with a reason

| Pinned | Tag | Why |
|---|---|---|
| Every shared `@github` layer; `build.yml`; the `charly` namespace import | `v2026.141.1600` (layers, `build.yml`) / `v2026.143.844` (`charly` namespace) | the ecosystem layer tag the shared refs pin to |
...
charly box build bazzite --include-disabled

# From the parent opencharly repo:
charly -C image/bootc image build bazzite --include-disabled

# Standalone, against the published repo:
charly --repo overthinkos/bootc image build bazzite --include-disabled
```
